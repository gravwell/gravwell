...
CONFIG_FILE=$ETC/simple_relay.conf
BINARY=$BIN/gravwell_simple_relay
...
installFiles() {
	...
	mkdir -p $CACHE
	mkdir -p $CRASH_LOG
	mv $TEMP/gravwell_simple_relay $BINARY
	...
	mv $TEMP/simple_relay.conf $CONFIG_FILE
}
...
stopService() {
	...
	if [ ! -f "/etc/systemd/system/gravwell_simple_relay.service" ]; then
		return
	fi
	/bin/systemctl stop gravwell_simple_relay.service 2>&1 > /dev/null
	/bin/systemctl disable gravwell_simple_relay.service 2>&1 > /dev/null 
}
...
checkService() {
	...
	FAILED="no"
	if [ ! -f "/etc/systemd/system/gravwell_simple_relay.service" ]; then
		/bin/systemctl status gravwell_simple_relay.service 2>&1 > /dev/null
...
installInit() {
	...
	mv $TEMP/gravwell_simple_relay.service /etc/systemd/system/gravwell_simple_relay.service
	/bin/systemctl enable gravwell_simple_relay.service 2>&1 > /dev/null
	if [ "$NO_START" != "yes" ]; then
		/bin/systemctl start gravwell_simple_relay.service
	fi
	echo "DONE"
}
...
extractIngestSecret() {
	if [ ! -f "$ETC/gravwell.conf" ]; then
		...
		if [ "$authVal" == "" ]; then
			...
			echo "   Restart the gravwell_simple_relay service afterwords"
			echo "*********************************************************************"
			echo
			return
		fi
