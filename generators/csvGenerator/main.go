...
var (
	tagName    = flag.String("tag-name", "csv", "Tag name for ingested data")
...
func init() {
	...
	if *srcOverride != `` {
		if src, err = getSource(*srcOverride); err != nil {
			log.Fatal(err)
		}
	} else {
...
func main() {
	...
	if err = igst.Sync(time.Second); err != nil {
		log.Fatal("Failed to flush ingest muxer ", err)
	}
...
func getSource(v string) (ip net.IP, err error) {
	//attempt to parse as IP
	if ip = net.ParseIP(v); ip != nil {
		return
	}
	//attempt to parse as hex string
	var b []byte
	...
	err = errors.New("Invalid src specifier")
	return
}
