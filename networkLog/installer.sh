...
CONFIG_FILE=$ETC/network_capture.conf
BINARY=$BIN/gravwell_network_capture
...
installFiles() {
	...
	mkdir -p $CACHE
	mkdir -p $CRASH_LOG
	mv $TEMP/gravwell_network_capture $BINARY
	...
	if [ "$ALT_CONFIG_FILE" != "" ]; then
		...
		echo "An existing config file exists at $TEMP/network_capture.conf"
	...
	mv $TEMP/network_capture.conf $CONFIG_FILE
}
...
stopService() {
	...
	if [ ! -f "/etc/systemd/system/gravwell_network_capture.service" ]; then
		return
	fi
	/bin/systemctl stop gravwell_network_capture.service 2>&1 > /dev/null
	/bin/systemctl disable gravwell_network_capture.service 2>&1 > /dev/null 
}
...
checkService() {
	...
	FAILED="no"
	if [ ! -f "/etc/systemd/system/gravwell_network_capture.service" ]; then
		/bin/systemctl status gravwell_network_capture.service 2>&1 > /dev/null
		if [ "$?" != "0" ] && [ "$NO_START" != "yes"]; then
...
installInit() {
	...
	mv $TEMP/gravwell_network_capture.service /etc/systemd/system/gravwell_network_capture.service
	/bin/systemctl enable gravwell_network_capture.service 2>&1 > /dev/null 
	if [ "$NO_START" != "yes" ]; then
		/bin/systemctl start gravwell_network_capture.service
	fi
	echo "DONE"
}
...
extractIngestSecret() {
	if [ ! -f "$ETC/gravwell.conf" ]; then
		...
		if [ "$authVal" == "" ]; then
			...
			echo "   Restart the gravwell_network_capture service afterwords"
			echo "*********************************************************************"
			echo
			return
		fi
...
setCaptureOwnership() {
	...
	if [ "$?" != "0" ]; then
		echo; echo;
		echo "*********************************************************"
		echo "We could not set the capability for binding to low ports."
		echo "The Gravwell simple relay will be limited to ports > 1024"
		echo "*********************************************************"
		echo; echo
	fi
}
