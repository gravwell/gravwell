...
func TestLoginNoMFA_script_mode(t *testing.T) {
	...
	t.Run("u/p login -> token login -> different u/p login", func(t *testing.T) {
		...
		} else if _, err := connection.Client.ListMacros(); err != nil {
			t.Fatal("failed to make call after logging in via credentials: ", err)
		}
...
func ping(t *testing.T, c *grav.Client) {
	t.Helper()
	if err := c.RefreshLoginToken(); err != nil {
		t.Fatal("failed to ping for default client")
	}
}
...
func verifyLoggedInStatus(expectedUsername string) error {
	...
	if _, err := connection.Client.ListMacros(); err != nil {
