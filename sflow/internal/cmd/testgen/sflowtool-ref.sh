...
for bin in "${bin_files[@]}"; do
	...
	if [[ -n "$output" ]]; then
		...
		set +e
		filtered=$(echo "$output" | jq 'del(.datagramSourceIP, .datagramSize, .unixSecondsUTC, .unixSecondsUTC_uS, .localtime)' 2>/dev/null)
		jq_status=$?
		set -e
		if [[ $jq_status -eq 0 ]]; then
			...
			echo "$output" >"$logfile"
			echo "jq failed (invalid JSON from sflowtool?), wrote output to $logfile" >&2
		fi
	else
